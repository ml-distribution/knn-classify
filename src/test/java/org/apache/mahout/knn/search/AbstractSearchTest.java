...
public abstract class AbstractSearchTest {
	...
	public void testExactMatch() {
		...
		for (MatrixSlice query : queries) {
			...
			assertEquals("Answer must be substantially the same as query", 0, r.get(0).getValue().minus(query.vector())
					.norm(1), 1e-8);
	...
	public void testRemoval() {
		...
		if (s instanceof UpdatableSearcher) {
			...
			assertEquals("Previous second neighbor should be first", 0, r.get(0).getValue().minus(r0.get(1).getValue())
					.norm(1), 1e-8);
