...
public class kNN {
	...
	private final DataSet dataFile; // the data set
	private int[] k; // holds the number of nearest neighbors
	private int[] kernel; // holds the kernel widths
	private final int trainSetSize; // size of the training set
	private final int testSetSize; // size of the final test set
	private final kNNSample[] trainSet; // the training set
	private final kNNSample[] testSet; // the final test set
	...
	**/
	public kNN(DataSet dataFile, int trainSetSize) {
		...
		trainSet = new kNNSample[trainSetSize];
		for (int i = 0; i < trainSetSize; i++)
			trainSet[i] = new kNNSample(dataFile.getExample(i), i);

		// Set up the test set
		this.testSetSize = dataFile.size() - trainSetSize;
		testSet = new kNNSample[testSetSize];
		int index = 0;
		for (int i = trainSetSize; i < dataFile.size(); i++) {
			testSet[index] = new kNNSample(dataFile.getExample(i), i);
			index++;
		}
	...
	**/
	public int[] train(int k[], int kernel[]) {
		...
		kNNSample queryExample; // the query example
		kNNSample[] subTrainSet = new kNNSample[trainSetSize - 1]; // the remaing train set
		int index = 0;
		for (int i = 0; i < trainSetSize; i++) {
			index = 0;
	...
	**/
	private double[][] testSingle(Sample testExample, kNNSample[] trainSet) {
		...
		kNNSample neighbor; // neighbor considered
	...
	**/
	private void scaleFeatureWeights(int mode) {
		...
		if (mode == 0)
			kNNSample.setFeatureWeights(featureWeights);
		// 1/(max-min)
		else if (mode == 1)
			kNNSample.setFeatureWeights(featureWeights1);
		// 1/var
		else if (mode == 2)
			kNNSample.setFeatureWeights(featureWeights2);
	}
