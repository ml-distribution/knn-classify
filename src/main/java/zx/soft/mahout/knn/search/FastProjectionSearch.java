...
public class FastProjectionSearch extends UpdatableSearcher {
	...
	private final List<List<WeightedThing<Vector>>> scalarProjections;
	...
	public List<WeightedThing<Vector>> search(Vector query, int limit) {
		...
		for (int i = 0; i < basisVectors.size(); ++i) {
			...
			int middle = Collections.binarySearch(currProjections, new WeightedThing<Vector>(null, projection));
			if (middle < 0) {
				middle = -(middle + 1);
			}
			for (int j = Math.max(0, middle - searchSize); j < Math
					.min(currProjections.size(), middle + searchSize + 1); ++j) {
		...
		List<WeightedThing<Vector>> top = Lists.newArrayListWithCapacity(candidates.size() + pendingAdditions.size());
