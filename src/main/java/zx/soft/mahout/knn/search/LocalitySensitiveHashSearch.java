...
public class LocalitySensitiveHashSearch extends UpdatableSearcher implements Iterable<Vector> {
	...
	private final Set<HashedVector> trainingVectors = Sets.newHashSet();
	...
	public List<WeightedThing<Vector>> search(Vector q, int numberOfNeighbors) {
		...
		PriorityQueue<WeightedThing<Vector>> top = new PriorityQueue<WeightedThing<Vector>>(getSearchSize(), Ordering
				.natural().reverse());
		...
		List<WeightedThing<Vector>> r = Lists.newArrayList(Iterables.transform(top,
				new Function<WeightedThing<Vector>, WeightedThing<Vector>>() {
					...
					public WeightedThing<Vector> apply(@Nullable WeightedThing<Vector> input) {
						return new WeightedThing<Vector>(((HashedVector) (input.getValue())).getVector(), input
								.getWeight());
					}
				}));
		Collections.sort(r);
	...
	public boolean remove(Vector v, double epsilon) {
		return trainingVectors.remove(new HashedVector(v, projection, HashedVector.INVALID_INDEX, BITMASK));
	}
