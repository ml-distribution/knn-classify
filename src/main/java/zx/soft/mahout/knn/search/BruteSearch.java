...
public class BruteSearch extends UpdatableSearcher {
	...
	public List<WeightedThing<Vector>> search(Vector query, int limit) {
		...
		PriorityQueue<WeightedThing<Integer>> bestNeighbors = new PriorityQueue<WeightedThing<Integer>>(limit, Ordering
				.natural().reverse());
		// The reulting list of weighted WeightedVectors (the weight is the distance from the query).
		List<WeightedThing<Vector>> results = Lists.newArrayListWithCapacity(limit);
		int rowNumber = 0;
		...
		for (int i = limit - 1; i >= 0; --i) {
			...
			results.set(i, new WeightedThing<Vector>(referenceVectors.get(neighbor.getValue()), neighbor.getWeight()));
		}
		return results;
	}
	 ...
	 */
	public List<List<WeightedThing<Vector>>> search(Iterable<WeightedVector> queries, final int limit, int numThreads) {
